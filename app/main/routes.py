...
from app import db
from app.main.forms import EditProfileForm, PostForm, SearchForm, EmptyForm, MessageForm
from app.models import User, Post, Message, Notification
from app.translates import translate
...
def before_request():
    ...
    g.locale = str(get_locale())  # para hacer que las fechas se traduzcan
...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash(_('Your post has been added!'))
...
def follow(username):
    ...
    if user is None:
        flash(_('ERROR: User not found'))
        return redirect(url_for('main.index'))
    if user == current_user:
        flash(_('ERROR: Same user.'))
